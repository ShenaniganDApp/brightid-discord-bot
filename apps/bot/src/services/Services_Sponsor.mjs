...
var RetryAsync = /* @__PURE__ */ Caml_exceptions.create(
  'Services_Sponsor.RetryAsync',
)
...
function noUnusedSponsorshipsOptions(param) {
  return {
    content:
      'There are no sponsorhips available in the Discord pool. Please try again later.',
    ephemeral: true,
  }
}
...
function makeAfterSponsorActionRow(label) {
  var verifyButton = new DiscordJs.MessageButton()
    .setCustomId('verify')
    .setLabel(label)
    .setStyle('PRIMARY')
...
var HandleSponsorError = /* @__PURE__ */ Caml_exceptions.create(
  'Services_Sponsor.HandleSponsorError',
)
...
async function checkSponsor(uuid) {
  ...
  var match = Json$JsonCombinators.decode(
    json,
    Decode$Shared.Decode_BrightId.Sponsorships.data,
  )
  var match$1 = Json$JsonCombinators.decode(
    json,
    Decode$Shared.Decode_BrightId.$$Error.data,
  )
  if (match.TAG === /* Ok */ 0) {
    return /* Sponsorship */ {
      _0: match._0.data,
...
async function handleSponsor(interaction, maybeHashOpt, attemptsOpt, uuid) {
  var maybeHash =
    maybeHashOpt !== undefined
      ? Caml_option.valFromOption(maybeHashOpt)
      : undefined
  ...
  try {
    var json = await Brightid_sdk_v5.sponsor(
      envConfig.sponsorshipKey,
      'Discord',
      uuid,
    )
    var err = Json$JsonCombinators.decode(
      json,
      Decode$Shared.Decode_BrightId.Sponsorships.sponsor,
    )
    if (err.TAG === /* Ok */ 0) {
    ...
    if (error.RE_EXN_ID === Js_exn.$$Error) {
      try {
        var brightIdError = Core__Option.map(
          Core__Option.map(JSON.stringify(error._1), function (prim) {
            return JSON.parse(prim)
          }),
          function (__x) {
            return Json$JsonCombinators.decode(
              __x,
              Decode$Shared.Decode_BrightId.$$Error.data,
            )
          },
...
var brightIdLinkVerificationEndpoint =
  Endpoints.brightIdLinkVerificationEndpoint

var makeCanvasFromUri = Commands_Verify.makeCanvasFromUri

var createMessageAttachmentFromCanvas =
  Commands_Verify.createMessageAttachmentFromCanvas
